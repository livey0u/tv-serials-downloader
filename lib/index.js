...
class Downloader {
  ...
  start() {
    return this.completePendingDownloads().then(() => this.fetchRecentReleases());
  }

  completePendingDownloads() {

    let incompleteDownloads = this.serialsCollection.find({ inprogress: true });

    return Promise.all(incompleteDownloads.map((serial) => this.download(serial)));
  ...
  download(serial) {
    ...
    if (existing && existing.inprogress) {
      ...
      let videoPath = path.join(path.resolve(__dirname, '../'), `${existing.name}-${existing.videoIndex}.mp4`);
    ...
    return new Promise((resolve, reject) => {
      ...
      let video = youtubedl(`http://www.youtube.com/watch?v=${serial.videoId}`, ['--format=18']);
      let videoPath = path.join(path.resolve(DOWNLOAD_LOCATION, `${serial.name}-${serial.videoIndex}.mp4`));
      video.on('info', function(info) {
        logger.info({ message: `Downloading ${ info.size / 1000000 } MB file to ${videoPath}`, ...serial });
      });


      let stream = video.pipe(fs.createWriteStream(videoPath));
